...
import { Autocomplete, Box, Button, Checkbox, Dialog, DialogActions, DialogContent, DialogTitle, FormControl, FormControlLabel, FormGroup, FormHelperText, FormLabel, Grid, IconButton, InputAdornment, ListItemText, MenuItem, Select, TextField, Typography, useTheme } from "@mui/material"
...
const validationSchema = yup.object().shape({
    ...
    //             programCount: yup.string().required('Program count is required'),
    //         })
    //     )
    //     .min(1, 'At least one Program Name and Count is required'),
...
const EditClgProfile = () => {
    ...
    const fetchEditData = async () => {
        ...
        reset({
            ...
            programCount: userData?.orgDetails?.collegePrograms?.length
                || null,
            totalAlumCount: userData?.orgDetails?.totalAlumCount || null,
            totalFacultyCount: userData?.orgDetails?.totalFacultyCount || null,
            totalStudCount: userData?.orgDetails?.totalStudCount || null,
            ...
            preferredFeatures: userData?.orgDetails?.preferredFeatures
                ...
                : null, // Change empty array to null

            integrationNeeds: userData?.orgDetails?.integrationNeeds ? userData?.orgDetails?.integrationNeeds.split(',').map((item) => item.trim().toLowerCase().replace(/ /g, '_')) : null,
            ...
            additionalComments: userData?.orgDetails?.additionalComments || null,
            submittedBy: userData?.orgDetails?.submittedBy || '',

            // collegePrograms: userData?.orgDetails?.collegePrograms?.map((program) => ({
            //     programName: program.programName,
            //     programCount: program.programCount,
            // })) || null,

            logo: userData?.orgDetails?.logo || null,
            brandingFile: userData?.orgDetails?.brandingFile || null,
        });

        const selectedCountry = countryList.find(
            (country) =>
                country.name?.toLowerCase() === userData?.orgDetails?.country?.toLowerCase().trim()
        );
            ...
            try {
                ...
                if (Array.isArray(states)) {
                    const selectedState = states.find(
                        (state) =>
                            state.name?.toLowerCase() === userData?.orgDetails?.state?.toLowerCase().trim()
                    );
                    if (selectedState) {
                        ...
                        const cities = await fetchCityData(
                            ...
                            selectedState.name
                        ); // Ensure cities are resolved
                        if (Array.isArray(cities)) {
                            const selectedCity = cities.find(
                                (city) =>
                                    city.name?.toLowerCase() === userData?.orgDetails?.city?.toLowerCase().trim()
                            );
                            if (selectedCity) {
    ...
    useEffect(() => {
        if (userData) {
            setCollegeProgram(userData?.orgDetails?.collegePrograms || []);
            setValue('collegePrograms', userData?.orgDetails?.collegePrograms || []);
    ...
    const onSubmit = async (data) => {
        ...
        // if (data.totalAlumCount) payload.append('totalAlumCount', userData?.orgDetails?.totalAlumCount || 0);
        // if (data.totalFacultyCount) payload.append('totalFacultyCount', userData?.orgDetails?.totalFacultyCount || 0);
        // if (data.totalStudCount) payload.append('totalStudCount', userData?.orgDetails?.totalStudCount || 0);
        ...
        // if (collegeProgram.length > 0) {
        //     collegeProgram.forEach((program, index) => {
        //         if (program.programName && program.programCount) {  // Ensure values exist before appending
        ...
        let endpoint = ApiEndPoints.AUTH.edit_college_profile;
        ...
        axiosInstance
            ...
            .then((response) => {
                ...
                setCollegeProgram([]); // Clear college programs
                navigate('/profile');
            })
            .catch((error) => {
                toastError(error);
    ...
    const handleAddCollegeProgram = () => {
        if (programNameInput.trim() && programCodeInput.trim()) {
            setValue('collegePrograms', [
                ...collegeProgram,
                {

                    programName: programNameInput.trim(),
                    programCount: programCodeInput.trim(),
                },
            ]);

            setCollegeProgram(prev => [
                ...prev,
                {
                    programName: programNameInput.trim(),
                    programCount: programCodeInput.trim(),
            ...
            clearErrors('collegePrograms');
            setProgramNameInput("");  // Reset input
            setProgramCodeInput("");  // Reset input
        }
    };

    const handleDeleteCollegeProgram = (programIndexToDelete) => {
        setCollegeProgram((prevCollegeProgram) => {
            const updatedCollegePrograms = prevCollegeProgram.filter((_, index) => index !== programIndexToDelete);

            // Update form value
            setValue('collegePrograms', updatedCollegePrograms);

            // Revalidate to show errors if no records exist
            trigger('collegePrograms');

            return updatedCollegePrograms;
        });
    };
    ...
    return (
        <>
            {/* <Box sx={{ p: 3, mb: 5 }}>
                <Typography variant="h4" fontWeight={800} sx={{ color: "primary.main" }}>Edit College</Typography>
            </Box> */}
            <Grid container sx={{ display: "flex", flexDirection: "column", p: { xs: 2, md: 10 } }} spacing={6}>
                <form id="org-form" onSubmit={handleSubmit(onSubmit)}>
                    <Grid container spacing={4}>
                                    ...
                                    render={({ field: { onChange, value } }) => (
                                        <Autocomplete
                                            ...
                                            onChange={(e, newValue) => {
                                                ...
                                                fetchCityData(selectedCountry?.name, newValue?.name); // Fetch cities by state name
                                            }}
                                            value={
                                                stateList.find((option) => option?.name === value) || null
                                            }
                                            getOptionLabel={(option) => option?.name || ''}
                                            renderInput={(params) => (
                                                <TextField
                        ...
                        <Grid item xs={12} sm={4}>
                            <FormControl fullWidth>
                                <FormLabel required htmlFor='adminFirstName'>Admin First Name</FormLabel>
                                <Controller
                                    name="adminFirstName"
                                    control={control}
                                    render={({ field: { value, onChange } }) =>
                        ...
                        <Grid item xs={12} sm={4}>
                            <FormControl fullWidth>
                                <FormLabel required htmlFor='adminLastName'>Admin Last Name</FormLabel>
                                <Controller
                                    name="adminLastName"
                                    control={control}
                                    render={({ field: { value, onChange } }) =>
                        ...
                        <Grid item xs={12} pb={3}>
                            <FormControl fullWidth>
                                            ...
                                            <Grid container spacing={2}>
                                                <Grid item xs={11} md={5.5}>
                                                    <TextField
                                                        ...
                                                        error={Boolean(errors.collegePrograms)}
                                                    />
                                                </Grid>
                                                <Grid item xs={11} md={5.5}>
                                                    <TextField
                                                        ...
                                                        variant="outlined"
                                                        onChange={(e) => setProgramCodeInput(e.target.value)}
                                                        id="numberOfProgram"
                                                ...
                                                <Grid item xs={1} md={1}>
                                                    <InputAdornment position="end">
                                                        <Button disableRipple onClick={handleAddCollegeProgram} sx={{ mt: 10 }} variant="contained" fullWidth>
                                                            Add
                                                        </Button>
                                                    </InputAdornment>
                                                </Grid>
                                ...
                                />
                                {errors.collegePrograms && (
                                    <FormHelperText sx={{ color: "error.main" }}>
                                        {errors.collegePrograms.message}
                                    </FormHelperText>
                                )}
                            </FormControl>
                        </Grid>
                        {collegeProgram?.length > 0 && (
                            <Grid item xs={12}>
                                <Box
                                    sx={{
                                        display: "flex",
                                ...
                                >
                                    {collegeProgram.map((desc, index) => (
                                        ...
                                        >
                                            ...
                                            <IconButton onClick={() => handleDeleteCollegeProgram(index)}>
                        ...
                        <Grid item xs={12} sm={6}>
                            <FormControl fullWidth>
                                <FormLabel required htmlFor='yearEstablished'>Year Established</FormLabel>
                                <Controller
                        ...
                        {/* <Grid item xs={4}>
                            <FormControl fullWidth>
                                <FormLabel required htmlFor='totalAlumCount'>Total Alumni Count</FormLabel>
                                <Controller
                                    name="totalAlumCount"
                                    control={control}
                                    rules={{ required: true, pattern: /^[0-9]+$/ }}
                                    render={({ field: { value, onChange } }) =>
                                        <TextField
                                            ...
                                            value={value}
                                            disabled
                                            error={Boolean(errors.totalAlumCount)}
                                            InputProps={{
                                                inputComponent: CleaveNumberInput,
                                                readOnly: true,
                                            }}
                                ...
                                />
                                {errors.totalAlumCount && (
                                    <FormHelperText sx={{ color: 'error.main' }}>
                                        {errors.totalAlumCount.message}
                                    </FormHelperText>
                                )}
                            </FormControl>
                        </Grid>
                        ...
                        <Grid item xs={4}>
                            <FormControl fullWidth>
                                <FormLabel required htmlFor='totalStudCount'>Total Student Count</FormLabel>
                                <Controller
                                    name="totalStudCount"
                                    control={control}
                                    rules={{ required: true, pattern: /^[0-9]+$/ }}
                                    render={({ field: { value, onChange } }) =>
                                        <TextField
                                            ...
                                            defaultValue={0}
                                            inputMode="numeric"
                                            placeholder='Enter Total Student Count'
                                            onChange={onChange}
                                            id='totalStudCount'
                                            value={value}
                                            disabled
                                            error={Boolean(errors.totalStudCount)}
                                            InputProps={{
                                                inputComponent: CleaveNumberInput,
                                                readOnly: true,
                                            }}
                                ...
                                />
                                {errors.totalStudCount && (
                                    <FormHelperText sx={{ color: 'error.main' }}>
                                        {errors.totalStudCount.message}
                                    </FormHelperText>
                                )}
                            </FormControl>
                        </Grid> */}
